...
export default function PCOSTrackerScreen() {
  ...
  const [insights, setInsights] = useState<PCOSInsights | null>(null);
  ...
  const loadData = useCallback(async () => {
    ...
    if (todayLog) {
      ...
      if (todayLog.cycleLength) setCycleLength(String(todayLog.cycleLength));
      setHasLoggedToday(true);
    } else {
      const init: Record<string, Severity> = {};
      PCOS_SYMPTOMS.forEach((s) => (init[s.id] = "none"));
      setSymptoms(init);
      setHasLoggedToday(false);
    }

    if (logs.length > 0) {
      setInsights(generatePCOSInsights(logs));
    }
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      last7.push({
        date: dateStr,
        score: log ? computeDailyPCOSScore(log) : 0,
      });
    }
    setWeekScores(last7);
  }, []);
                ...
                <View>
                  {insights.alerts.map((alert, i) => (
                    <View
                      ...
                      style={{
                        backgroundColor: alert.startsWith("💡")
                    ...
                    >
                      ...
                      <Text
                        ...
                        style={{
                          color: alert.startsWith("💡") ? "#7c3aed" : "#b91c1c",
                        }}
                      >
                        {alert}
                      </Text>
            ...
            {weekScores.map((item, i) => {
              ...
              return (
                <View key={i} className="items-center flex-1">
                  ...
                  />
                  <Text
                    className={`text-[10px] mt-1 ${isToday ? "font-bold text-purple-500" : "text-slate-400"}`}
                  >
                    {dayLabels[dayIdx]}
                  </Text>
                </View>
        ...
        <View className="bg-white rounded-2xl p-5 border border-slate-100 mb-5">
          ...
          {PCOS_CATEGORIES.map((cat) => (
            <View key={cat.id} className="mb-5">
              ...
              {getSymptomsByCategory(cat.id).map((symptom) => (
                <View key={symptom.id} className="mb-3">
              ...
              {cat.id !== "mood" && <View className="h-px bg-slate-100 mt-2" />}
            </View>
          ))}
